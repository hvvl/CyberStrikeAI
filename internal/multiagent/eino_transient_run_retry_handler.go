...
// channelID 当前 run 使用的 AI 通道 ID（用于重试进度展示；空 = 未标识）。
func (h *einoTransientRunRetryHandler) channelID() string {
	if h == nil || h.cfg.Args == nil {
		return ""
	}
	return h.cfg.Args.channelID()
}
...
func emitEinoRunRetryProgress(
	...
	progress("eino_run_retry", retryMsg, data)
