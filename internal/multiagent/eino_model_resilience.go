...
func newEinoToolCallingChatModelFactory(
	...
	return func(ctx context.Context, oa config.OpenAIConfig, mode einoModelMode) (model.ToolCallingChatModel, error) {
		...
		httpClient := channelAwareLLMHTTPClient(&oa, baseHTTPClient, logger)
...
// channelAwareLLMHTTPClient 在上游共享 baseHTTPClient 之上，按 oa.ChannelID 叠加
// 通道级 transport 链：Retry-After cooldown → 并发限流器（两者均为轻量包装，
// 不复制连接池；同一通道的多个模型共享同一 gate 与 cooldown 注册表）。
// ChannelID 为空（如遗留配置直接用 openai: 段）时直通 baseHTTPClient。
func channelAwareLLMHTTPClient(oa *config.OpenAIConfig, base *http.Client, logger *zap.Logger) *http.Client {
	if base == nil {
		base = newEinoBaseHTTPClient()
	}
	if oa == nil || strings.TrimSpace(oa.ChannelID) == "" {
		// 无通道标识：保留上游原有 claude bridge + summarization 诊断链。
		hc := openai.NewEinoHTTPClient(oa, base)
		openai.AttachSummarizationDiagTransport(hc, logger)
		return hc
	}
	client := &http.Client{
		Timeout:   base.Timeout,
		Transport: &retryAfterTransport{channelID: oa.ChannelID, next: channelLimiterTransport(oa.ChannelID, oa.MaxConcurrency, base.Transport)},
	}
	// claude bridge（若启用）+ summarization 诊断，与上游工厂保持一致。
	hc := openai.NewEinoHTTPClient(oa, client)
	openai.AttachSummarizationDiagTransport(hc, logger)
	return hc
}

// newEinoAgenticModelRetryConfigForChannel 按通道级 run_retry_* 覆盖生成原生 ModelRetry 配置。
// 未配置通道级覆盖时与 newEinoAgenticModelRetryConfig 等价。
func newEinoAgenticModelRetryConfigForChannel(
	mw *config.MultiAgentEinoMiddlewareConfig,
	oa *config.OpenAIConfig,
	logger *zap.Logger,
	scope string,
) *adk.TypedModelRetryConfig[*schema.AgenticMessage] {
	p := einoTransientRunRetryPolicyForChannel(mw, oa, channelIDOf(oa))
		...
		ShouldRetry: func(ctx context.Context, retryCtx *adk.TypedRetryContext[*schema.AgenticMessage]) *adk.TypedRetryDecision[*schema.AgenticMessage] {
			...
			if retryCtx.Err != nil {
				...
				if logger != nil {
					...
					logger.Warn("eino native agentic model retry",
						zap.String("scope", scope),
						zap.String("channel", p.channelID),
						zap.Int("attempt", retryCtx.RetryAttempt),
						zap.Int("maxRetries", maxRetries),
						zap.String("errorKind", kind),
						zap.String("errorSummary", summary),
...
// channelIDOf 安全提取通道 ID。
func channelIDOf(oa *config.OpenAIConfig) string {
	if oa == nil {
		return ""
	}
	return strings.TrimSpace(oa.ChannelID)
}
