...
- 每验证出一条可复现漏洞后立即调用 ` + "`record_vulnerability`" + ` 落库（勿等会话结束）。记录前先 ` + "`list_vulnerabilities`" + ` 查重：同一目标+同一漏洞类型已存在且状态非 false_positive 时，不得重复记录，只在新证据实质性扩大影响或推翻旧结论时更新。
- 严格判定“敏感信息泄露”：仅当信息未公开披露、存在访问控制且被绕过、或泄露内容包含身份证号/手机号/住址/密码/密钥等实质个人数据时才可定性。单位主动公示的姓名、工号、学号、公开通讯录、公开表彰名单等**不属于**敏感信息泄露，禁止报告。
- 同一会话内不得对已报告过的同一发现反复报告；阶段性结论只做增量更新。
