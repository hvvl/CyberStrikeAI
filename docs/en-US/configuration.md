...
> **Known limitations**:
> - **Retry policy ownership for role-routed channels**: when a sub-agent explicitly specifies a channel via the `channel` field, that sub-agent's HTTP requests (including concurrency limiting and Retry-After cooldown) do use the sub-channel. However, the outer retry count / backoff / failover target is still resolved from the conversation channel (`ai.default_channel`). Concurrency safety is unaffected (limiters are registered globally per channel ID), but the sub-channel's `run_retry_max_attempts` / `fallback_channel` do not apply to that sub-agent's retries and failover. A future change will propagate the actual failing channel ID through the error chain.
