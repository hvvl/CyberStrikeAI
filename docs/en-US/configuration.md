...
| `run_retry_max_attempts` | Channel-level retry count for transient errors (rate limit / 5xx / network jitter), exponential backoff; 0 = inherit global `multi_agent.eino_middleware.run_retry_max_attempts`, default 6 retries. |
...
- **Retry count & backoff**: default 6 retries (excluding the initial call), 30s per-retry backoff cap (equal-jitter exponential backoff). Global values live at `multi_agent.eino_middleware.run_retry_max_attempts` / `run_retry_max_backoff_sec`; channel-level fields (table above) take precedence.
- **Retry-After**: when a response carries `Retry-After` (429/503 etc.), that channel enters cooldown for the specified duration (hard-capped at 120s); waiting does not consume the channel's concurrency quota.
- **Cross-channel failover**: with `fallback_channel` configured, exhausting retries automatically switches to the backup channel and resumes from the completed progress (segment-resume, at most once; cyclic `fallback_channel` chains are cut off). The failing channel is attributed at the HTTP transport layer — 429/5xx responses and network-level transient errors (connection reset/refused, dial, DNS, TLS timeout, etc.) are all recorded into a **per-run collector** (concurrent conversations never cross-attribute): when a role-routed channel fails, **its own `fallback_channel` takes priority and only that role channel is redirected** — the conversation's main channel (model/key/endpoint/concurrency/retry policy) stays untouched; agents whose Markdown pins `channel: <failed>` resolve to the backup (including `fallback_channel` pointing back at the conversation channel, e.g. `cheap.fallback_channel: strong`). If the failed channel has no `fallback_channel`, the conversation channel's failover config applies. When the conversation channel itself fails, the whole session switches.

> **Known limitations**:
> - **Retry count ownership for role-routed channels**: when a sub-agent explicitly specifies a channel via the `channel` field, that sub-agent's HTTP requests (concurrency limiting, Retry-After cooldown) and its failover target are handled per sub-channel. However, the outer retry count / backoff cadence is still resolved from the conversation channel (`ai.default_channel`) because the run loop is a conversation-level retry layer. The sub-channel's `run_retry_max_attempts` / `run_retry_max_backoff_sec` take effect when that channel is the conversation channel.
> - **Background resume execution identity**: the background worker for "continue failed conversations" executes the agent as the conversation owner, consistent with the established `batch_queue_executor` delegation pattern. Stricter privilege isolation (e.g., inheriting the requesting user's permissions) would require changes across all background batch paths. Resume runs on the **AI channel bound to the conversation** (`conversations.ai_channel_id`, persisted at creation / last use); if an admin later deleted that channel, resume explicitly reports it and falls back to the default instead of silently switching models.
> - **Post-failover iteration budget**: each segment of a cross-channel resume re-reads `agent.max_iterations` as the per-segment cap, so the total iteration count across segments may reach 2× the configured value. `max_iterations` is a per-segment limit, not a hard task-wide cap.
- **Continue failed conversations**: the "Continue failed conversations" button next to "Recent conversations" in the chat sidebar lists conversations interrupted by exhausted retries on transient LLM API failures (rate limits / upstream 5xx / network flakiness). Error messages carry a machine-readable `[api_failure:*]` tag; non-retryable errors are excluded. Resume one or all via the background queue; progress streams through task events. APIs: `GET /api/agent-loop/failed`, `POST /api/agent-loop/continue-failed` (requires `tasks:write`, consistent with batch task management).
