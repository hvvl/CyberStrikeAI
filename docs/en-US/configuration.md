...
- **Cross-channel failover**: with `fallback_channel` configured, exhausting retries automatically switches to the backup channel and resumes from the completed progress (segment-resume, at most once; cyclic `fallback_channel` chains are cut off). The failing channel is attributed at the HTTP transport layer (429/5xx responses): when a role-routed channel fails, **its own `fallback_channel` takes priority** and, after failover, that channel is redirected to the backup for the rest of the request (agents whose Markdown pins `channel: <failed>` also resolve to the backup). If the failed channel has no `fallback_channel`, the conversation channel's failover config applies.

> **Known limitations**:
> - **Retry count ownership for role-routed channels**: when a sub-agent explicitly specifies a channel via the `channel` field, that sub-agent's HTTP requests (concurrency limiting, Retry-After cooldown) and its failover target are handled per sub-channel. However, the outer retry count / backoff cadence is still resolved from the conversation channel (`ai.default_channel`) because the run loop is a conversation-level retry layer. The sub-channel's `run_retry_max_attempts` / `run_retry_max_backoff_sec` take effect when that channel is the conversation channel. Additionally, when multiple channels fail concurrently across conversations, failing-channel attribution uses a "most recent failure" heuristic and a rare cross-conversation misattribution is possible.
